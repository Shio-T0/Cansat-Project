...
function init() {
    ...
    const createVector = (color) => {
        ...
        const cylinderMat = new THREE.MeshBasicMaterial({ color: color, transparent: true, opacity: 0.8 });
...
function updateUI(rx, ry, rz) {
    ...
    const mapBar = (rad) => Math.min(100, Math.max(0, ((rad + Math.PI) / (Math.PI * 2)) * 100));
...
function animate() {
    ...
    if (isAutoOrbit) {
        ...
        updateUI(planet.rotation.x % (Math.PI*2), planet.rotation.y % (Math.PI*2), planet.rotation.z % (Math.PI*2));
    }
...
function setupInteractions() {
    ...
    document.getElementById('reset-btn').addEventListener('click', () => {
        gsap.to(planet.rotation, { x: 0, y: 0, z: 0, duration: 1.5, ease: "expo.out" });
    });
    ...
    // Initial Animations
    gsap.from(".animate-fade-in", { opacity: 0, y: -20, duration: 1, delay: 0.5 });
    gsap.from(".animate-slide-up", { opacity: 0, y: 50, duration: 1, delay: 0.8 });
}
